...
// GET — list all active surveys with question count + whether user responded
...
export async function POST(req: NextRequest) {
  ...
  const { title, description, department, questions } = await req.json() as {
    ...
    description?: string
    department?: string
    questions: Array<{ question: string; type: string; options?: string[]; required?: boolean; sort_order?: number }>
  }
