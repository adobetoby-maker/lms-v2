...
export function VideoLibraryPicker({ courseId, onClose, onAdded }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-3xl max-h-[85vh] flex flex-col rounded-2xl border border-[#2a2a4a] bg-[#0d0d1a] shadow-2xl">
        ...
        {/* Filters */}
        <div className="flex gap-3 px-5 py-3 border-b border-[#1e1e3a]">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500" />
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search videos…"
              className="w-full pl-9 pr-4 py-2 rounded-xl bg-[#0a0a18] border border-[#2a2a4a] text-slate-200 text-sm placeholder:text-slate-600 outline-none focus:border-indigo-500 transition-colors" />
          </div>
          <div className="relative">
            <select value={topic} onChange={e => setTopic(e.target.value)}
              className="appearance-none pl-3 pr-8 py-2 rounded-xl bg-[#0a0a18] border border-[#2a2a4a] text-slate-300 text-sm outline-none focus:border-indigo-500 transition-colors cursor-pointer">
                ...
                {vids.map(v => (
                  <div key={v.id}
                    className={`relative rounded-xl border overflow-hidden transition-all ${
                      added.has(v.id) ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-[#1e1e3a] bg-[#0a0a18] hover:border-indigo-500/40'
                    }`}>
        ...
        <div className="px-5 py-3 border-t border-[#1e1e3a] flex items-center justify-between">
          ...
          <button onClick={onClose} className="px-4 py-1.5 rounded-lg bg-[#1a1a2e] text-slate-300 text-sm hover:text-white transition-colors">
            Done
          </button>
        </div>
      </div>
